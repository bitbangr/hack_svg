    ...
    for row in 0..rows {
        for col in 0..cols {
            if !visited[row as usize ][col as usize] {
                ...
                let color : &String = &mosaic_nd_arr[row as usize][col as usize].1;
...
fn dfs_arr(mosaic_nd_arr: &ndarray::ArrayBase<ndarray::OwnedRepr<(Box2D<i32, UnknownUnit>, RGB)>, ndarray::Dim<[usize; 2]>>, row: isize, col: isize, color: color, visited: &[Vec<bool, Global>], rows: isize, cols: isize) 
                    -> Vec<(isize, isize)> {
